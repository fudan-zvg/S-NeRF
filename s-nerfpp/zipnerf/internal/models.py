...
class Model(nn.Module):
    ...
    Args:
      ...
      cal_input_grad: bool, True for pose refine only, cause we need the grad from hash grid
...
class MLP(nn.Module):
    ...
    grid_disired_resolution: int = 8192  # 8192
    ...
    def __init__(self, **kwargs):
        ...
        if not self.disable_rgb:
            ...
            # self.sem_layer = nn.Sequential(nn.Linear(self.bottleneck_width, 256), # v3
            #                                nn.ReLU(),
            #                                nn.Linear(256, self.class_num))

    def predict_density(self, means, stds, return_weights=False, rand=False, cal_input_grad=False):
        ...
        if self.warp_fn is not None:
            ...
            means = means / bound
            stds = stds / bound
        features = self.encoder(means, bound=1, cal_input_grad=cal_input_grad).unflatten(-1, (self.encoder.num_levels, -1))

        # import pdb; pdb.set_trace()

        # if features.shape[-2] > 1 :
        #     high_feq_mask = torch.zeros_like(features, device=features.device)
        #     # high_feq_mask[...,:4,:] = 1
        #     features = features*high_feq_mask
    ...
    """
        if self.disable_density_normals:
            raw_density, x = self.predict_density(means, stds, rand=rand, cal_input_grad=cal_input_grad)
