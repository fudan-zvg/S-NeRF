...
def resample_linear_spline(t, tp, vp):
    '''
    Args:
        t:  tensor with shape (..., n+1)
        tp: tensor with shape (..., m+1)
        vp: knots tensor with shape (..., m), while vp_0[...,0]=1 after we cat 0 to the vp

        but I simplified the interplotation to a quadratic function while treated it as a linear function
    Returns:

    '''
    vp_0 = torch.cat([torch.zeros(vp.shape[:-1]+(1,), device=vp.device), vp], -1)
    weights = (vp_0[...,1:]+vp_0[...,:-1])/2*(tp[...,1:]-tp[...,:-1])
    acc = torch.cumsum(weights, dim=-1)
    acc0 = torch.cat([torch.zeros(acc.shape[:-1] + (1,), device=acc.device), acc], dim=-1)
    acc0_resampled = math.sorted_interp(t, tp, acc0)  # TODO
    v = torch.diff(acc0_resampled, dim=-1)
    return v
