...
def set_diff(mask_A, mask_B):
    '''
    return mask_A - mask_B.
    '''
    array_A = np.array(mask_A) > 0
    array_B = np.array(mask_B) > 0
    
    result_mat = np.logical_and(array_A, np.logical_not(np.logical_and(array_A, array_B)))
    result_mat = result_mat.astype(np.uint8) * 255
    return Image.fromarray(result_mat)
...
def scale(im,
    ...
    # Padding to avoid our vehicle getting outside.
    im_mat = np.pad(im_mat, [(2*h, 2*h), (2*w, 2*w), (0, 0)])
    mask_im_mat = np.pad(mask_im_mat, [(2*h, 2*h), (2*w, 2*w), (0, 0)])
    ...
    im_final_mat = im_final_mat[(2*h):(3*h), (2*w):(3*w), :]
    mask_im_final_mat = mask_im_final_mat[(2*h):(3*h), (2*w):(3*w), :]
    ################################
    
    return Image.fromarray(im_final_mat), Image.fromarray(mask_im_final_mat)
