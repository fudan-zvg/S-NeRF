...
def load_colmap_depth(basedir, factor=8, bd_factor=.75):
    ...
    #* Y:down X:right Z:forward
    #p_xyz = np.array([pt[1].xyz for pt in points.items()])
    # import pdb; pdb.set_trace()
    # from mmdet3d.core.visualizer import show_result
    # show_result(p_xyz, None, None, out_dir='./vis/', filename='raw_vis', show=False)
    ...
    for id_im in range(1, len(images)+1):
        ...
        for i in range(len(images[id_im].xys)):
            ...
            if id_3D == -1:
                continue
            point3D = points[id_3D].xyz # Image [id_im] / Pixel [i] Corresponded Point
