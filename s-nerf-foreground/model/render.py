...
def render_path(H, W, render_poses, intrinsics, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=1, render_depth=[], render_dist=None):
    render_bds = []
    if(render_dist):
        render_bds = [(render_dist*0.7, render_dist*1.3)] * len(render_poses)
    else:
        for dep_img in render_depth:
            valid_dep = dep_img[dep_img!=0]
            near = valid_dep.min().item() 
            far = valid_dep.max().item()
            bds = (near, far)
            render_bds.append(bds)
    ...
    if render_factor != 1:
        ...
        if(len(render_depth)):
            render_mask_new = []
            for depth_img in render_depth:
                img = cv2.resize(depth_img, (int(W/render_factor), int(H/render_factor)))
                render_mask_new.append(img)
            render_mask = torch.tensor(np.stack(render_mask_new))
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        if(i < len(render_bds)-1):
            near, far = render_bds[i]
            render_kwargs['near'], render_kwargs['far'] = near*0.95, far*1.05
        intr = intrinsics[i] if len(intrinsics) > 1 else intrinsics[0]

        ori_point = [intr[0,2] / render_factor, intr[1,2] / render_factor] 
        focal = (intr[0,0]+intr[1,1]) / (2*render_factor)
        
        rgb, disp, acc, depth, extras = render(H, W, focal, ori_points=ori_point, chunk=chunk, c2w=c2w[:3,:4], retraw=True, **render_kwargs)
        
        rgbs.append(rgb.cpu().numpy())
        deps.append(depth.cpu().numpy())
...
def create_nerf(args):
    ...
    return render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, ckpt
